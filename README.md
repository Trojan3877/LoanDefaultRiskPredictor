...
| Reproducibility context | Captures data fingerprint, training commit, split seed, trial count, Python/package versions, and wall-clock training time. | `dataset_id`, `training_commit`, `protocol`, `environment`, `training_seconds` |
...
Never compare results from different dataset fingerprints, training commits, split methods, seeds, thresholds, or package versions as though they were the same experiment. Review calibration, cohort evidence, and fair-lending obligations separately before any real-world use.
