...
The v0.2.0 audit raised the enforced coverage floor from 75% to **80%** and added a CI-generated benchmark evidence artifact. The v0.3.0 release workflow validates an immutable semantic tag, re-runs the quality gates, builds source and wheel distributions, creates GitHub provenance attestations, publishes a signed GHCR image, and attaches checksums/SBOM/evidence to the GitHub Release.
...
pip-audit --cache-dir .pip-audit-cache --requirement requirements/runtime.txt
python -m pip install build twine
python -m build
twine check dist/*
```

If a locked-down Windows profile cannot create pytest's default temporary directory, use an in-repository ignored location:

```powershell
pytest --basetemp .testtmp --cov --cov-report=term-missing --cov-fail-under=80
```
...
After all pull-request checks are green and the change is merged, create an immutable tag whose version matches `src/version.py`; only then does the release workflow run:

```bash
git checkout main
git pull --ff-only
git tag -a v0.3.0 -m "Loan Default Risk Predictor v0.3.0"
git push origin v0.3.0
```

The tag publisher creates:

- `ghcr.io/coreyleath-code/loan-default-risk-predictor:v0.3.0`;
...
- an sdist and wheel, each validated by Twine and attested with GitHub build provenance;
- `sbom.spdx.json`;
- `release-evidence.txt`;
- `release-checksums.txt`;
- an immutable GitHub Release tag `v0.3.0`.

The workflow validates that the tag matches package metadata and refuses to overwrite an existing GitHub Release. It publishes to PyPI only when the repository administrator configures the protected `pypi` environment for [PyPI trusted publishing](https://docs.pypi.org/trusted-publishers/) and sets `PUBLISH_PYPI=true`; no long-lived package token is stored in the workflow.
...
The detailed engineering scorecard and remaining risks are documented in [`docs/AUDIT.md`](docs/AUDIT.md), with the current release decision and reproducibility evidence in [`docs/PRODUCTION_READINESS_AUDIT_2026-08-26.md`](docs/PRODUCTION_READINESS_AUDIT_2026-08-26.md). The governing principle is simple: **README, code, CI, release artifacts, and model evidence must agree.**
